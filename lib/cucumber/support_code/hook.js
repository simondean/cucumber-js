var Hook = function(code, options) {
  ...
  var self = Cucumber.SupportCode.StepDefinition(HookStepDefinition.EMPTY_PATTERN, code);
  var tags = options['tags'] || [];
  var scenario;

  self.setScenario = function setScenario(newScenario) {
  ...
  self.buildInvocationParameters = function buildInvocationParameters(step, callback) {
    if (code.length === 1)
      return [callback];
    else
